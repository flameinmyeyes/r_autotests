...
public class FUN_02_BP_006_MT_1_KP_2_1 extends HooksTSE_DEMO_28080 {



    /**
     * МТ 1. КП 2(1). Формирование записи в справочнике  "Проекты" исполнителем ЦС ФК
     */

    public static  String WAY_TEST = Ways.TSE_DEMO_28080.getWay() + "\\FUN_02\\BP_006\\FUN_02_BP_006_MT_1_KP_2_1\\";


    @Description("МТ 1. КП 2(1). Формирование записи в справочнике  \"Проекты\" исполнителем ЦС ФК")
    @Owner(value = "Каверина Марина")
    @Link(name="TSE-T1795", url="https://job-jira.otr.ru/secure/Tests.jspa#/testCase/TSE-T1795")
    ...
    public static void step04() {
       ...
       В поле "Наименование проекта" ввести  ЕДПОСТАВЩИК
       В поле "Примечание" ввести Проекты казначейского сопровождения
       Выбрать чек-бокс Федеральный
       Поле "Дата начала действия записи " заполнено текущей датой
       Поле "Статус " заполнено значением Новая
         */
        String docDate;
        CommonFunctions.printStep();
        String rand = "ЕДПОСТАВЩИК_АТ_" + CommonFunctions.randomNumber(100000, 999999);
    ...
    public static void step05() {
        ...
        //D разделе колонок у "Уровень бюджета" у колонка "Федеральный" заполнено значением Да, колонка "Бюджет субъекта " заполнено значением Нет, колонка "Муниципальный " заполнено значением Нет
        Assert.assertEquals($x("//*[contains(@class,\"addedDocument\")]//td[3]").getAttribute("title"), "Да");
        Assert.assertEquals($x("//*[contains(@class,\"addedDocument\")]//td[4]").getAttribute("title"), "Нет");
        Assert.assertEquals($x("//*[contains(@class,\"addedDocument\")]//td[5]").getAttribute("title"), "Нет");
