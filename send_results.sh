#!/bin/bash
#echo "------------------SEND-TO-JIRA------------------"
#curl -k --basic --user api-jira:a7fR48xH -F "file=@d:\Projects\Jira\tm4j_result.zip" https://job-jira.otr.ru/rest/atm/1.0/automation/execution/FSSP

# This directory is where you have all your results locally, generally named as `allure-results`
ALLURE_RESULTS_DIRECTORY='target/allure-results'
# This url is where the Allure container is deployed. We are using localhost as example
ALLURE_SERVER='http://172.31.197.147:5050'
# Project ID according to existent projects in your Allure container - Check endpoint for project creation >> `[POST]/projects`
PROJECT_ID='purks-tests'
...
#curl -X POST "$ALLURE_SERVER/allure-docker-service/generate-report?project_id=default"
...
EXECUTION_NAME='autorun'
EXECUTION_FROM='gitlab'
EXECUTION_TYPE='auto'

#You can try with a simple curl
curl -X GET "$ALLURE_SERVER/allure-docker-service/generate-report?project_id=$PROJECT_ID&execution_name=$EXECUTION_NAME&execution_from=$EXECUTION_FROM&execution_type=$EXECUTION_TYPE"
